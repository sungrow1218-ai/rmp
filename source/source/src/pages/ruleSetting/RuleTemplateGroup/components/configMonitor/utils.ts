...
export function renderMultiThreshold(
  ...
  return template.replace(/\$(\d+)/g, (match, factorTypeStr) => {
    ...
    if (threshold && threshold.value && typeof threshold.value === 'object') {
      // 根据因子类型确定后缀
      let suffix = '';
      if (factorType === 15 || factorType === 35) suffix = '万元';
      if (factorType === 16 || factorType === 36) suffix = '万股';
      ...
      // 获取三级值
      const notipValue = threshold.value[RiskLevel.NOTIP];
      const warningValue = threshold.value[RiskLevel.WARNING];
      const interceptValue = threshold.value[RiskLevel.INTERCEPT];
