...
// 判断是否使用模拟数据 - 强制使用真实API
const useMockData = false; // 强制禁用模拟数据

// 动态导入模拟服务
let mockAuthService: any = null;
...
export async function userLogin(params: LoginParamType) {
  // 如果使用模拟数据且模拟服务已加载
  if (useMockData && mockAuthService) {
    return mockAuthService.userLogin(params);
  }
...
export async function userLogout() {
  // 如果使用模拟数据且模拟服务已加载
  if (useMockData && mockAuthService) {
    return mockAuthService.userLogout();
  }
...
export async function validateToken() {
  // 如果使用模拟数据且模拟服务已加载
  if (useMockData && mockAuthService) {
    return mockAuthService.validateToken();
  }
...
export async function userInfo() {
  // 如果使用模拟数据且模拟服务已加载
  if (useMockData && mockAuthService) {
    return mockAuthService.userInfo();
  }
...
export async function getUserRoles(params: RequestParameterPagination) {
  // 如果使用模拟数据且模拟服务已加载
  if (useMockData && mockAuthService) {
    return mockAuthService.getUserRoles(params);
  }
...
export async function getCurrentUserRoles(params: RequestParameterPagination) {
  // 如果使用模拟数据且模拟服务已加载
  if (useMockData && mockAuthService) {
    return mockAuthService.getCurrentUserRoles(params);
  }
