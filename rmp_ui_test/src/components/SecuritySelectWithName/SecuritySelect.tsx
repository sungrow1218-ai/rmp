...
const SecuritySelectWithName: FC<IProps> = ({
  onChange,
  value = [],
  securityType,
  style = { width: '100%' },
      ...
      try {
        ...
        setOptions(
          resultList.map((i) => ({
            ...
            value: `${i.securityCode}|${i.marketId}|${i.securityName}`,
          }))
        );
      } catch (error) {
        console.error(error);
  ...
  const fetch = async (search?: string) => {
    ...
    try {
      ...
      if (value && value.length !== 0) {
        for (const { securityCode, marketId, securityName } of value) {
          const option = getOptions().find(
            (i) => i.value === `${securityCode}|${marketId}|${securityName}`
          );
          if (option) {
      ...
      onChange &&
        onChange(
          [...realValue, item.value].map((i) => {
            const [securityCode, marketId, securityName] = i.split('|');
            return { securityCode, marketId: Number(marketId), securityName };
          })
        );
    } else {
      onChange &&
  ...
  const renderSelectedOptionItem = (item: OptionItem) => (
    <div
      aria-selected="true"
      className="riskControlPlatform-select-item riskControlPlatform-select-item-option riskControlPlatform-select-item-option-selected custom-option"
      title={item.label}
      onClick={() => handleSelect(false, item)}
      key={item.value}
    >
      <div className="riskControlPlatform-select-item-option-content">
        {item.label}
      </div>
      <span
        className="riskControlPlatform-select-item-option-state"
        unselectable="on"
        aria-hidden="true"
      ...
      >
        <span
          role="img"
          aria-label="check"
          className="spriteicon spriteicon-check"
        >
          <svg
  ...
  const renderOptionItem = (item: OptionItem) => (
    <div
      aria-selected="false"
      className="riskControlPlatform-select-item riskControlPlatform-select-item-option custom-option"
      title={item.label}
...
export default SecuritySelectWithName;
