...
export const useFormEvents = (options: Options) => {
  ...
  const appendSchemaByField = async (
    ...
    const index = formatSchemas.findIndex(
      (schema) => schema.field === prefixField
    );
    const schemaList = isPlainObject(schema)
      ? [schema as FormSchema]
      : (schema as FormSchema[]);
    if (!prefixField || index === -1 || first) {
      // eslint-disable-next-line @typescript-eslint/no-unused-expressions
      first
        ? formatSchemas.unshift(...schemaList)
        : schemaList.push(...schemaList);
    } else if (index !== -1) {
