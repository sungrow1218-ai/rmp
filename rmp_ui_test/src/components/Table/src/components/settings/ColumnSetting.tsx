...
import { Tooltip, Popover, Checkbox, Divider, Button } from '@ht/sprite-ui';
import type { CheckboxChangeEvent } from '@ht/sprite-ui/lib/checkbox';
import {
  SettingOutlined,
  DragOutlined,
  ArrowLeftOutlined,
...
import {
  getPopupContainer as getPopupContainerFn,
  nextTick,
} from '@/utils/dom';
...
import { CheckboxGroupProps } from '@ht/sprite-ui/es/checkbox';
...
const ColumnSetting = (props: IProps) => {
  ...
  const defaultRowSelection =
    useRef<Nullable<TableRowSelection<Recordable>>>(null);
  ...
  // 列可选项
  const [columnOptions, setColumnOptions, getColumnOptions] = useGetState<
    ColumnOptionsType[]
  >([]);
  ...
  // 全选
  const [isColumnAllSelected, setIsColumnAllSelected] =
  ...
  const indeterminate = useCreation(() => {
    return (
      columnCheckedOptions.length > 0 &&
      columnCheckedOptions.length < columnOptions.length
    );
  ...
  const tableColumnsUpdate = () => {
    ...
    let count = columns.filter(
      (o) =>
        o.flag !== INDEX_COLUMN_FLAG && (o.fixed === 'left' || o.fixed === true)
    ).length;
  ...
  const isColumnAllSelectedUpdate = (options?: ColumnOptionsType[]) => {
    nextTick(() => {
      setIsColumnAllSelected(
        (options ?? columnOptions).length === columnCheckedOptions.length
      );
    });
  };
  // 更新 showIndexColumn
  ...
  const init = async () => {
    if (!isInit.current) {
      ...
      // 默认值 缓存
      defaultIsIndexColumnShow.current =
        tableContext.allProps.showIndexColumn || false;
        ...
        title={
          <div className={`${prefixCls}__popover-title`}>
            ...
            {defaultIsRowSelectionShow.current && (
              <Checkbox
                checked={isRowSelectionShow}
                onChange={onRowSelectionShowChange}
              >
                勾选列
              </Checkbox>
            )}
        ...
        content={
          ...
          >
            {columnOptions.map((option, index) => (
              <div
                key={index}
                className={`${prefixCls}__check-item`}
                data-no={option.value}
              >
