...
import { ColumnType } from '@ht/sprite-ui/lib/table';
...
export const useColumns = (
  ...
  useEffect(() => {
    ...
    _propsColumns.forEach((item) => {
      ...
      handleItem(
        item,
        Reflect.has(item, 'ellipsis') ? !!item.ellipsis : !!ellipsis && !render
      );
    });
  ...
  const setColumnsFn = (
    ...
    if (!isString(firstColumn) && !isArray(firstColumn)) {
      ...
      const columnKeys = (_columns as (string | string[])[]).map((m) =>
        m.toString()
      );
...
const handleIndexColumn = (
  props: Pick<
    BasicTableProps,
    'isTreeTable' | 'showIndexColumn' | 'indexColumnProps'
  >,
  pagination: PaginationProps | boolean,
  columns: BasicColumn[]
) => {
