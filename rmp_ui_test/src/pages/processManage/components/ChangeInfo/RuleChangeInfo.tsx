...
const getTableData = (data: any) => {
  ...
  Array.isArray(data) &&
    data?.forEach((lp: any) => {
      lp?.paramList?.forEach((tp: any) => {
        list.push({ ...tp, moduleName: lp.moduleName, key: uuidv4() });
      });
    });
  let currentModule = '';
  let currentCount = 0;
  return list.map((item: any, index: number) => {
    if (item.moduleName !== currentModule) {
      ...
      currentCount = list.filter(
        (it: any) => it.moduleName === currentModule
      ).length;
      return { ...item, row: currentCount };
    } else {
      return { ...item, row: 0 };
    }
  });
...
const RuleChangeInfo: React.FC<Props> = ({ changeText, size }) => {
  ...
  return (
    <div style={{ paddingBottom: '20px' }}>
      <Table
        size="middle"
        dataSource={getTableData(changeText)}
        columns={columns}
        rowKey={'key'}
        pagination={false}
