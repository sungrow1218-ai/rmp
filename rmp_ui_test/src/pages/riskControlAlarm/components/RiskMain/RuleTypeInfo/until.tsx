...
export const warnValue = (data: any, isT = 3, unit: string) => {
  if (
    ...
    if (isT === 1 && typeof data.warnThreshold === 'number') {
      ...
      const colorString = value + unit;
      return <>，触达阈值={<>{colorSpan(colorString)}</>}</>;
    }
    ...
    if (isT === 3 && typeof data.warnThreshold === 'number') {
      ...
      if (unit === '倍') {
        const colorString = value.toFixed(2) + unit;
        return <>，触达阈值={<>{colorSpan(colorString)}</>}</>;
      }
      const colorString = value + unit;
      return <>，触达阈值={<>{colorSpan(colorString)}</>}</>;
...
export const showIndexValue = (
  ...
  if (isT === 1 && typeof data === 'number') {
    ...
    return <>{isWarn ? colorSpan(`${value}${unit}`) : `${value}${unit}`}</>;
  }
  ...
  if (isT === 3 && typeof data === 'number') {
    ...
    if (unit === '倍') {
      ...
      return <>{isWarn ? colorSpan(`${value1}${unit}`) : `${value1}${unit}`}</>;
    }
    return <>{isWarn ? colorSpan(`${value}${unit}`) : `${value}${unit}`}</>;
  }
  return '--';
