import { queryPreciousMetal } from '@/services/securityInfo';
...
const PreciousMetalSelectSimple = forwardRef<IAction, IProps>(
  ({ value, onChange, marketId, style = { width: '240px' } }, ref) => {
    ...
    const onSearch = debounce(async (searchValue: string) => {
      ...
      try {
        ...
        const result = await queryPreciousMetal(params);
        if (result.errorId !== 0) {
