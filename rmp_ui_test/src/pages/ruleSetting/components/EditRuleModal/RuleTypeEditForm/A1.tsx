...
import { convertToObject } from './F1';
...
const EditFormZ01201: React.FC<Props> = ({
  ...
  const [ruleControlSecurity, setRuleControlSecurity] =
    useState<any>(undefined);
  // 监听股票可转债
  const [ruleCalculate192Values, setRuleCalculate192Values] = useState<any>({
    calParam192Values: '0',
  });
  // 监听计算参数改变
  const onRuleCalculateParamChange = (
    ...
    setRuleCalculateParamValues(allValues[19997] ?? undefined);
    setRuleCalculate192Values({
      calParam192Values: allValues[192] ?? '0',
    });
  };
  // 监听控制证券改变
  ...
  useEffect(() => {
    if (mode !== 'ADD') {
      ...
      if (!isEmpty(record?.ruleControlSecurity)) {
        ...
        setRuleControlSecurity({
          ...record?.ruleControlSecurity,
          securitySetIdList:
            record?.ruleControlSecurity.securityControlType === 10 &&
            isArray(record?.ruleControlSecurity.securitySetIdList) &&
            record?.ruleControlSecurity.securitySetIdList.length > 0
              ? record?.ruleControlSecurity.securitySetIdList[0]
              : record?.ruleControlSecurity.securitySetIdList,
        });
      }
    } else {
      setRuleControlSecurity(undefined);
    }
  }, [record, mode]);
  ...
  return (
    <>
      ...
      <RuleSecurityControl
        ...
        defaultValues={ruleControlSecurity ?? undefined}
        onValuesChange={onRuleSecurityParamChange}
        ruleParamValues={ruleCalculate192Values}
      />
      <RuleCalculateParam
        mode={mode}
        ruleType={ruleType}
