      ...
      securityGroupList: m.security_group_list.map((s) => {
        ...
        return {
          ...
          thresholdList: s.forbid_threshold_list.map((i) => {
            const lowValue = sValue?.thresholdList.find(
              (t) =>
                t.factorType === i.factor_type &&
                t.warnLevel === RiskLevel.NOTIP
            );
            const middleValue = sValue?.thresholdList.find(
              (t) =>
                t.factorType === i.factor_type &&
                t.warnLevel === RiskLevel.WARNING
            );
            const highValue = sValue?.thresholdList.find(
              (t) =>
                t.factorType === i.factor_type &&
                t.warnLevel === RiskLevel.INTERCEPT
            );
            return {
              thresholdId: lowValue ? lowValue.thresholdId : undefined,
              warnLevel: undefined,
              compareDirection: lowValue
                ? lowValue.compareDirection
                : i.compare_direction,
              value:
                lowValue && middleValue && highValue
                  ? {
                      [RiskLevel.NOTIP]: lowValue.value,
                      [RiskLevel.WARNING]: middleValue?.value,
                      [RiskLevel.INTERCEPT]: highValue?.value,
                    }
                  : undefined,
              factorType: lowValue ? lowValue.factorType : i.factor_type,
              unit: lowValue ? lowValue.unit : i.unit,
            };
          }),
...
function isRecordable(value: any): value is Recordable<number> {
  ...
  for (const market of tmpl.marketList.filter((i) =>
    ...
    for (const securityGroup of market.securityGroupList) {
      ...
      for (const thresholdItem of securityGroup.thresholdList) {
        if (thresholdItem.value === null || thresholdItem.value === undefined) {
          throw Error(
            `请完整配置规则类型：${transformDictCodeToNameHelper(
              tmpl.ruleType,
              RULE_TYPE_LEVEL_2
            )}（${tmpl.ruleType}）`
          );
        ...
        item.thresholdList.push({
          ...
          warnLevel: RiskLevel.NOTIP,
          compareDirection: thresholdItem.compareDirection,
          value: thresholdItem.value
            ? isRecordable(thresholdItem.value)
              ? thresholdItem.value[RiskLevel.NOTIP]
              : thresholdItem.value
            : undefined,
        ...
        item.thresholdList.push({
          ...
          value: thresholdItem.value
            ? isRecordable(thresholdItem.value)
              ? thresholdItem.value[RiskLevel.WARNING]
              : thresholdItem.value
            : undefined,
          unit: thresholdItem.unit,
          factorType: thresholdItem.factorType,
        ...
        item.thresholdList.push({
          ...
          value: thresholdItem.value
            ? isRecordable(thresholdItem.value)
              ? thresholdItem.value[RiskLevel.INTERCEPT]
              : thresholdItem.value
            : undefined,
          unit: thresholdItem.unit,
          factorType: thresholdItem.factorType,
